        ...
        <div class="d-flex flex-column-fluid">
            <div class="container">
                <div class="card card-custom">
                    ...
                    <div class="card-body">
                        <form action="{{ route('kasir.user.save') }}" method="post" autocomplete="off">
                            @csrf
                            @if(!empty($user))
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <a href="{{ route('kasir.user') }}" class="btn btn-secondary font-weight-bold">Batal</a>
                                </div>
                            </div>
                        </form>
                    </div>
                    @if(!empty($user))
                        <div class="card-footer text-right p-2">
                            <form action="{{ route('kasir.user.delete') }}" method="post" id="form_hapus">
                                @csrf
                                <input type="hidden" name="id" value="{{ $user->id }}">

        ...
        <div class="d-flex flex-column-fluid">
            <div class="container">
                <div class="card card-custom">
                    ...
                    <div class="card-body">
                        <form action="{{ route('kasir.produk.save') }}" method="post" autocomplete="off">
                            @csrf
                            @if(!empty($produk))
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="nama" name="nama" value="{{ !empty($produk) ? $produk->nama : '' }}" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="harga">Harga</label>
                                        <input type="text" class="form-control autonumeric" id="harga" name="harga" value="{{ !empty($produk) ? $produk->harga : '' }}" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="kode">Kode (opsional)</label>
                                        <input type="text" class="form-control" id="kode" name="kode" value="{{ !empty($produk) ? $produk->kode : '' }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="kategori">Kategori (opsional)</label>
                                        <input type="text" class="form-control" id="kategori" name="kategori" value="{{ !empty($produk) ? $produk->kategori : '' }}">
                                    </div>
                                    <button type="submit" class="btn btn-primary font-weight-bold">Simpan</button>
                                    <a href="{{ route('kasir.produk') }}" class="btn btn-secondary font-weight-bold">Batal</a>
                                </div>
                            </div>
                        </form>
                    </div>
                    @if(!empty($produk))
                        <div class="card-footer text-right p-2">
                            <form action="{{ route('kasir.produk.delete') }}" method="post" id="form_hapus">
                                @csrf
                                <input type="hidden" name="id" value="{{ $produk->id }}">

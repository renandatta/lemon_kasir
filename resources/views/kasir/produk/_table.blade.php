<div class="table-responsive">
    <table class="table">
        ...
        @foreach($produk as $key => $value)
            <tr>
                <td>
                    {{ $value->nama . ($value->kode != '' ? ' ('. $value->kode .')' : '') }}<br>
                    <span class="text-muted">{{ $value->kategori }}</span>
                </td>
                <td class="text-right">{{ format_number($value->harga) }}</td>
                @if(in_array('edit', $action))
                    <td class="p-1 text-right">
                        <a href="{{ route('kasir.produk.info', 'id=' . $value->id) }}" class="btn btn-sm btn-default btn-text-primary btn-hover-primary btn-icon" title="Edit details">
